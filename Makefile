...
# 版本化标签: <tag>-<YYYYMMDD>-<git-sha>，dirty 工作区追加 -dirty
# 例如 local-20260620-249ba63 / local-20260620-249ba63-dirty
BUILD_DATE      ?= $(shell date +%Y%m%d)
GIT_SHA         ?= $(shell cd $(CURDIR) && git rev-parse --short HEAD 2>/dev/null || echo unknown)
GIT_DIRTY       := $(shell cd $(CURDIR) && test -n "$$(git status --porcelain 2>/dev/null)" && echo -dirty)
...
build-no-cache: ## 无缓存构建 (同时打 :local 与版本化标签)
