...
class ChatCompletionRequest(BaseModel):
    ...
    verbosity: Optional[Literal["low", "medium", "high"]] = Field(
        ...
        "produce more concise answers, higher values produce more thorough ones.",
    )
    modalities: Optional[List[Literal["text", "audio"]]] = Field(
        default=None,
        description="Output types the model should generate. Most models default to "
        "['text']. Audio output is not produced by the SGLang backend and is accepted "
        "for OpenAI API compatibility only.",
    )
    audio: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Parameters for audio output. Accepted for OpenAI API "
        "compatibility; audio output is not generated by the SGLang backend.",
    )
    prediction: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Static predicted output content (Predicted Outputs). Accepted "
        "for OpenAI API compatibility.",
    )
    web_search_options: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Options for the web search tool. Accepted for OpenAI API "
        "compatibility.",
    )
    moderation: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Configuration for running moderation on the request. Accepted "
        "for OpenAI API compatibility.",
    )
    metadata: Optional[Dict[str, str]] = Field(
        default=None,
        description="Set of up to 16 key-value pairs that can be attached to the "
        "request and returned on the response object.",
    )
    store: Optional[bool] = Field(
        default=None,
        description="Whether to store the output for use in model distillation or "
        "evals. Accepted for OpenAI API compatibility.",
    )
    service_tier: Optional[
        ...
        "Echoed back on the response for OpenAI API compatibility.",
    )
    prompt_cache_key: Optional[str] = Field(
        default=None,
        description="Used by OpenAI to cache responses for similar requests. Accepted "
        "for OpenAI API compatibility.",
    )
    prompt_cache_retention: Optional[Literal["in_memory", "24h"]] = Field(
        default=None,
        description="The retention policy for the prompt cache. Accepted for OpenAI "
        "API compatibility.",
    )
    safety_identifier: Optional[str] = Field(
        default=None,
        description="A stable identifier used to help detect users that may be "
        "violating usage policies. Supersedes the deprecated 'user' field.",
    )
    # Deprecated OpenAI fields, superseded by tools / tool_choice. Accepted so older
    # clients are not rejected.
    functions: Optional[List[Dict[str, Any]]] = Field(
        default=None,
        deprecated="functions is deprecated in favor of the tools field",
    )
    function_call: Optional[Union[Literal["none", "auto"], Dict[str, Any]]] = Field(
        default=None,
        deprecated="function_call is deprecated in favor of the tool_choice field",
    )
    task: Optional[
    ...
    def normalize_reasoning_inputs(cls, values: Dict):
        ...
        if r is not None and isinstance(r, dict):
            ...
            # Promote the OpenAI-defined effort levels only; the sglang-only "max"
            # tier stays a top-level reasoning_effort extension.
...
class ChatCompletionResponse(BaseModel):
    ...
    usage: UsageInfo
    # OpenAI response object fields, echoed for API compatibility.
...
class ResponseReasoningParam(BaseModel):
    ...
    generate_summary: Optional[Literal["auto", "concise", "detailed"]] = Field(
        default=None,
        deprecated="generate_summary is deprecated in favor of the summary field",
    )
...
class ResponsesRequest(BaseModel):
    ...
    # Conversation/context-management fields accepted for OpenAI API compatibility;
    # SGLang does not yet act on them.
    ...
    # Responses-style stream options (e.g. {"include_obfuscation": bool}); accepted
    # for OpenAI API compatibility.
    stream_options: Optional[Dict[str, Any]] = None
    temperature: Optional[float] = None
    # Configuration for a text response from the model (e.g. output format).
    text: Optional[Dict[str, Any]] = None
    # Accept object forms (function selection, allowed_tools, hosted tools) as well
    # as the string presets; downstream only special-cases the string values.
...
class ResponsesResponse(BaseModel):
    ...
    def from_request(
        ...
        return cls(
            ...
            # Honor an explicit text config if the client sent one, otherwise fall
            # back to the inferred plain-text format.
            text=request.text or text_format,  # TODO(v): Expand coverage per https://platform.openai.com/docs/api-reference/responses/list
